...
let filteredByBornDate = inventors.filter(inventor => inventor.year >= 1500 && inventor.year < 1600);
...
let sortedInventorsByBirthDate = inventors.sort((x,y)=> {return x.year - y.year});
...
let initialValue = 0;
let sumOfYears = inventors.reduce((acc, y)=>{return  acc + (y.passed - y.year)}, initialValue);
console.log('sumOfTheYears', sumOfYears);

//Sort the inventors by years lived

let sortedByYearsLived = inventors.sort((x,y)=> {return (x.passed - x.year)-(y.passed - y.year)});
console.log('sortedByyearsLived',sortedByYearsLived);

//Sort the ​ people​ alphabetically by last name

let sortedPeople = people.sort((x,y)=> {return x-y} );
